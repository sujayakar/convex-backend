//! V8 isolate environment for the SDK client simulation.
//!
//! Loads the `sdk_client.js` bundle and provides deterministic time/RNG
//! via `TestRuntime`. This is structurally identical to the js_client
//! environment but loads a different JS bundle.
...
pub const SDK_CLIENT_SOURCE: &str =
    include_str!("../../../../../npm-packages/simulation/dist/sdk_client.js");
pub const SDK_CLIENT_SOURCE_MAP_STR: &str =
    include_str!("../../../../../npm-packages/simulation/dist/sdk_client.js.map");
pub static SDK_CLIENT_SOURCE_MAP: LazyLock<SourceMap> = LazyLock::new(|| {
    SourceMap::from_slice(SDK_CLIENT_SOURCE_MAP_STR.as_bytes()).expect("Invalid source map")
});

pub struct SdkClientEnvironment {
    rt: TestRuntime,
    rng: ChaCha12Rng,
...
impl IsolateEnvironment<TestRuntime> for SdkClientEnvironment {
    async fn lookup_source(
        &mut self,
        path: &str,
        _timeout: &mut Timeout<TestRuntime>,
    ) -> anyhow::Result<Option<(Arc<FullModuleSource>, ModuleCodeCacheResult)>> {
        if path != "sdk_client.js" {
            return Ok(None);
        }
        Ok(Some((
            Arc::new(FullModuleSource {
                source: SDK_CLIENT_SOURCE.into(),
                source_map: Some(SDK_CLIENT_SOURCE_MAP_STR.to_string()),
            }),
    ...
    fn syscall(&mut self, _name: &str, _args: JsonValue) -> anyhow::Result<JsonValue> {
        panic!("syscall() unimplemented for SDK client");
    }
    ...
    fn trace(&mut self, level: LogLevel, messages: Vec<String>) -> anyhow::Result<()> {
        for message in messages {
            match level {
                LogLevel::Debug => tracing::debug!("[sdk_client] {message}"),
                LogLevel::Error => tracing::error!("[sdk_client] {message}"),
                LogLevel::Warn => tracing::warn!("[sdk_client] {message}"),
                LogLevel::Info => tracing::info!("[sdk_client] {message}"),
                LogLevel::Log => tracing::info!("[sdk_client] {message}"),
            }
        }
        Ok(())
    }
    ...
    fn get_all_table_mappings(&mut self) -> anyhow::Result<NamespacedTableMapping> {
        panic!("get_all_table_mappings() unimplemented for SDK client");
    }
